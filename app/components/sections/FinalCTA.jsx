"use client";
import { motion, useInView } from "framer-motion";
import { useRef } from "react";
import { ArrowRight, MessageCircle, Phone, Shield, Clock, Star } from "lucide-react";

const WA_LINK = "https://wa.me/919003360494?text=Hi%20Viya%20Nexus%2C%20I%20want%20to%20book%20a%20free%20strategy%20call";

export default function FinalCTA() {
  const ref = useRef(null);
  const isInView = useInView(ref, { once: true, margin: "-80px" });

  /* Shared button style */
  const btnBase = {
    width: "200px", height: "50px",
    display: "inline-flex", alignItems: "center", justifyContent: "center", gap: "8px",
    fontFamily: "var(--font-mono)", fontSize: "12px", fontWeight: 700,
    textTransform: "uppercase", letterSpacing: "0.1em",
    textDecoration: "none", cursor: "pointer",
    transition: "all 0.3s ease",
  };

  return (
    <section style={{ position: "relative", padding: "100px 0 80px", overflow: "hidden" }} ref={ref}>
      <div style={{ position: "absolute", inset: 0, background: "linear-gradient(135deg, #8B6000, #C9910A 60%, #E8B84B)" }} />
      {/* Texture overlay */}
      <div style={{ position: "absolute", inset: 0, background: "radial-gradient(circle at 20% 50%, rgba(0,0,0,0.1) 0%, transparent 50%)", pointerEvents: "none" }} />

      <div className="container-main" style={{ position: "relative", zIndex: 10, textAlign: "center" }}>
        {/* Social proof */}
        <motion.div initial={{ opacity: 0 }} animate={isInView ? { opacity: 1 } : {}} transition={{ delay: 0.1 }}
          style={{ display: "flex", justifyContent: "center", gap: "4px", marginBottom: "20px" }}>
          {[1,2,3,4,5].map(i => <Star key={i} size={16} fill="#06080D" style={{ color: "#06080D" }} />)}
          <span style={{ fontFamily: "var(--font-mono)", fontSize: "11px", color: "#06080D", marginLeft: "8px", fontWeight: 700 }}>Rated 5/5 by 30+ businesses</span>
        </motion.div>

        <motion.h2 initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5 }}
          style={{
            fontFamily: "var(--font-display)", fontWeight: 700, color: "#06080D",
            fontSize: "clamp(28px, 5vw, 56px)", lineHeight: 1.1,
            letterSpacing: "-0.02em", marginBottom: "12px",
          }}>
          Ready to stop doing it manually?
        </motion.h2>
        <motion.p initial={{ opacity: 0 }} animate={isInView ? { opacity: 1 } : {}} transition={{ delay: 0.15 }}
          style={{
            color: "rgba(6,8,13,0.65)", fontSize: "clamp(15px, 1.5vw, 18px)",
            maxWidth: "500px", margin: "0 auto 32px",
            fontFamily: "var(--font-body)", lineHeight: 1.7,
          }}>
          Book a free 30-minute strategy call. We&apos;ll show you exactly how much time and money you can save.
        </motion.p>

        {/* Buttons — IDENTICAL SIZE */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.25 }}
          style={{ display: "flex", gap: "0", justifyContent: "center", flexWrap: "wrap" }}>
          <a href={WA_LINK} target="_blank" rel="noopener noreferrer" className="cta-btn-animate cta-pulse"
            style={{ ...btnBase, background: "#06080D", color: "#E8B84B" }}>
            Book Free Call <ArrowRight size={14} />
          </a>
          <a href={WA_LINK} target="_blank" rel="noopener noreferrer" className="cta-btn-animate"
            style={{ ...btnBase, background: "transparent", color: "#06080D", boxShadow: "inset 0 0 0 2px #06080D" }}>
            <MessageCircle size={14} /> WhatsApp Us
          </a>
        </motion.div>

        {/* Trust signals row */}
        <motion.div initial={{ opacity: 0 }} animate={isInView ? { opacity: 1 } : {}} transition={{ delay: 0.4 }}
          style={{
            display: "flex", flexWrap: "wrap", justifyContent: "center", gap: "24px",
            marginTop: "32px", paddingTop: "24px",
            borderTop: "1px solid rgba(6,8,13,0.1)",
          }}>
          {[
            { icon: Shield, text: "No credit card required" },
            { icon: Clock, text: "Reply within 2 hours" },
            { icon: Phone, text: "7-day delivery" },
          ].map((t, i) => {
            const Icon = t.icon;
            return (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                <Icon size={12} style={{ color: "rgba(6,8,13,0.4)" }} />
                <span style={{ fontFamily: "var(--font-mono)", fontSize: "9px", color: "rgba(6,8,13,0.45)", textTransform: "uppercase", letterSpacing: "0.08em" }}>{t.text}</span>
              </div>
            );
          })}
        </motion.div>
      </div>
    </section>
  );
}
